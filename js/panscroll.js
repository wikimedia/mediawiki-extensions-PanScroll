$(document).ready(function () {
    ...
    function enablePanScrolling(elem, event) {
        ...
        $(elem).on('mousemove', function (mmevent) {
            ...
            var maxTop = -1 * (10 + $(elem).outerHeight(true) - $(elem).parent().outerHeight(true));
    ...
    $('.panscroll-element').each(function () {
        ...
        var styleHeight = $(this)[0].style.height;

        var contentWidth = $(this).children('.panscroll-container').outerWidth(true);
        var contentHeight = $(this).children('.panscroll-container').outerHeight(true);
